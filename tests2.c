...
// See if reuse all space.
int test7() {
#define ARRAY_SIZE 40
    int cpt = 0;
    int *tab[ARRAY_SIZE];
    int *tab2[ARRAY_SIZE];
    int *ptr = mymalloc(sizeof(int)*1024*1024);
    myfree(ptr);
    for(int i = 0; i < ARRAY_SIZE; ++i) {
        tab[i] = mymalloc(sizeof(int)*1024*1024);
    }
    for(int i = 0; i < ARRAY_SIZE; ++i) {
        myfree(tab[i]);
    }
    ...
    for(int i = 0; i < ARRAY_SIZE; ++i) {
        tab2[i] = mymalloc(sizeof(int)*1024*1024);
    }
    for(int i = 0; i < ARRAY_SIZE; ++i) {
        if(tab[i] == tab2[i]) {
            cpt++;
